...
let Shape = {
	...
	expand(shape, left, top, width, height, opt_relativeCoords) {
	...
	// Also mutate coords in the old grid system to move to the new grid system.
	reduce(shape, opt_relativeCoords) {
		var w = shape.width;
	...
	newIndexTransformer(shape, i, j, viewWidth, opt_checkValid) {
	...
	getGridFits(grid, shape, negInfo, opt_blacklist, opt_blacklistOffset) {
		...
		for (var i = -negInfo.width;
			...
			for (var j = -negInfo.height;
				...
				var gridTransformer = Shape.newIndexTransformer(grid, i, j, shape.width, negativesRemaining);
	...
	setOnGrid(grid, coord, value, opt_addlTransform) {
		var i = coord.i,
			j = coord.j,
			shape = coord.shape;
		var gridTransformer = Shape.newIndexTransformer(grid, i, j, shape.width, true);
		for (var index = 0; index < shape.width * shape.height; index++) {
			if (shape.grid[index]) {
				var gridIndex = gridTransformer(index);
	...
	setMultiple(shape) {
		...
		shape.multiple = !!grid.find(function(val, index) {
